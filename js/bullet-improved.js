// Improved Bullet class with enhanced homing
class Bullet extends Phaser.Physics.Arcade.Sprite {
    constructor(scene, x, y, program = []) {
        ...
        // Enhanced homing properties
        this.homingTarget = null;
        this.homingStrength = 0.08; // Increased from 0.05 for better tracking
        this.homingRange = 300; // Range to detect enemies
    ...
    applyProperty(propertyNode) {
        switch (propertyNode.action) {
            case 'homing':
                ...
                this.createHomingTrail();
                console.log('Applied homing property - enhanced tracking enabled');
                break;
    ...
    createHomingTrail() {
        try {
            // Create a more visible homing trail
            const trail = this.scene.add.particles(this.x, this.y, this.texture.key, {
                ...
                tint: 0xFF69B4, // Pink trail
                follow: this
            });
            
            this.activeEffects.push(trail);
            
            // Add pulsing ring effect for homing
            const homingRing = this.scene.add.circle(this.x, this.y, 12, 0xFF69B4, 0);
            homingRing.setStrokeStyle(2, 0xFF69B4, 0.6);
            this.activeEffects.push(homingRing);
            ...
            console.warn('Could not create homing trail:', error);
        }
    }
    ...
    // Enhanced homing implementation
    updateHoming() {
        ...
        // Find closest enemy within range, or keep current target if valid
        if (!this.homingTarget || !enemies.includes(this.homingTarget)) {
            let closestEnemy = null;
            let closestDistance = Infinity;
            
            enemies.forEach(enemy => {
                const distance = Phaser.Math.Distance.Between(this.x, this.y, enemy.x, enemy.y);
                if (distance < closestDistance && distance <= this.homingRange) {
        ...
        if (this.homingTarget) {
            ...
            const targetX = this.homingTarget.x;
            const targetY = this.homingTarget.y;
            
            // Predict enemy movement if it has velocity
            if (this.homingTarget.body && this.homingTarget.body.velocity) {
                const predictionTime = 0.3; // 300ms prediction
                const predictedX = targetX + this.homingTarget.body.velocity.x * predictionTime;
                const predictedY = targetY + this.homingTarget.body.velocity.y * predictionTime;
                
                const targetAngle = Phaser.Math.Angle.Between(this.x, this.y, predictedX, predictedY);
                const currentAngle = Phaser.Math.Angle.Between(0, 0, this.body.velocity.x, this.body.velocity.y);
                
                // Enhanced turning with variable strength based on distance
                const distance = Phaser.Math.Distance.Between(this.x, this.y, targetX, targetY);
                const turnRate = this.homingStrength * (1 + Math.min(1, 100 / distance)); // Stronger turning when closer
                ...
                // Visual feedback when actively homing
                if (Math.abs(angleDiff) > 0.1) { // If turning significantly
                    this.createHomingSparkle();
                }
            }
        }
    }
    ...
    createHomingSparkle() {
        try {
            // Create small sparkle effect when homing is active
            const sparkle = this.scene.add.circle(this.x, this.y, 3, 0xFFFFFF, 0.8);
            this.scene.tweens.add({
                targets: sparkle,
                scaleX: 0.1,
                scaleY: 0.1,
            ...
            console.warn('Could not create homing sparkle:', error);
        }
    }
...
// Enhanced bullet factory function
function createProgrammedBullet(scene, x, y, direction, program) {
    ...
    scene.time.delayedCall(10, () => {
        if (bullet.body && bullet.isActive) {
            ...
            // Log if homing is active
            if (bullet.properties.homing) {
                console.log('🎯 Homing bullet created with enhanced tracking!');
            }
        } else {
